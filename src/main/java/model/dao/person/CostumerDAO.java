...
import model.entity.person.Costumer;
...
public class CostumerDAO extends PersonDAO {
    ...
    public static boolean create(Costumer costumer) {
        ...
        String sql = "INSERT INTO costumer (status_costumer, id_person) VALUES (?, ?)";
        PreparedStatement stmt = null;

        try {
            stmt = conn.prepareStatement(sql);
            stmt.setBoolean(1, costumer.isStatus());
    ...
    public static boolean update(Costumer costumer) {
        ...
        Connection conn = ConnectionFactory.getConnection();
        String sql = "UPDATE costumer SET status_costumer = ? WHERE id_person = ?";
        PreparedStatement stmt = null;

        try {
            stmt = conn.prepareStatement(sql);
            stmt.setBoolean(1, costumer.isStatus());
            stmt.setInt(2, costumer.getId());
            stmt.executeUpdate();
            return true;
        }
        catch (SQLException sqlE) {
    ...
    public static Costumer createInstance(ResultSet result) {
        Costumer costumer = null;

        try {
            costumer = new Costumer();
            costumer.setId(result.getInt("id_person"));
            costumer.setStatus(result.getBoolean("status_costumer"));
            costumer.setNamePerson(result.getString("name_person"));
            costumer.setAddress( AddressDAO.createInstance(result));
            costumer.setPhones(PhoneDAO.load(costumer));
        }
        catch (SQLException sqlE) {
            Logger.getLogger(CostumerDAO.class.getName()).log( Level.SEVERE, null, sqlE);
        }
